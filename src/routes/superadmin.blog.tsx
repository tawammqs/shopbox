...
function SuperadminBlogPage() {
  ...
  return <div className="space-y-6">
    ...
    <div className="flex gap-2">
      <Button variant={tab === "posts" ? "default" : "outline"} size="sm" onClick={() => setTab("posts")}>Posts</Button>
      <Button variant={tab === "leads" ? "default" : "outline"} size="sm" onClick={() => setTab("leads")}>Leads</Button>
    </div>
    {tab === "posts" ? <>
    ...
    <div className="overflow-hidden rounded-lg border bg-card"><Table><TableHeader><TableRow><TableHead>Título</TableHead><TableHead>Categoria</TableHead><TableHead>Status</TableHead><TableHead>Views</TableHead><TableHead>Data</TableHead><TableHead className="text-right">Ações</TableHead></TableRow></TableHeader><TableBody>
      {query.isLoading && <TableRow><TableCell colSpan={6} className="h-32 text-center"><Loader2 className="mx-auto h-5 w-5 animate-spin" /></TableCell></TableRow>}
      {!query.isLoading && posts.length === 0 && <TableRow><TableCell colSpan={6} className="h-32 text-center text-muted-foreground">Nenhum artigo encontrado.</TableCell></TableRow>}
    ...
    </TableBody></Table></div></> : <div className="overflow-hidden rounded-lg border bg-card"><Table><TableHeader><TableRow><TableHead>WhatsApp</TableHead><TableHead>Fonte</TableHead><TableHead>Data</TableHead></TableRow></TableHeader><TableBody>
      {leadsQuery.isLoading && <TableRow><TableCell colSpan={3} className="h-32 text-center"><Loader2 className="mx-auto h-5 w-5 animate-spin" /></TableCell></TableRow>}
      {!leadsQuery.isLoading && (leadsQuery.data ?? []).length === 0 && <TableRow><TableCell colSpan={3} className="h-32 text-center text-muted-foreground">Nenhum lead captado ainda.</TableCell></TableRow>}
      {(leadsQuery.data ?? []).map((lead) => <TableRow key={lead.id}><TableCell className="font-medium">{lead.whatsapp}</TableCell><TableCell>{lead.source}</TableCell><TableCell>{new Date(lead.created_at).toLocaleString("pt-BR")}</TableCell></TableRow>)}
    </TableBody></Table></div>}
