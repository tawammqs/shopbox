...
function VideoTestimonialsSection({ storeId, storeSlug, section }: { storeId: string; storeSlug: string; section: TheShoesSettings["video_section"] }) {
  ...
  const activeThumb = activeImages[0]?.url ?? "";
  ...
  return (
    <section className="ts-section">
      ...
      {active && <VideoPlayerModal video={active} product={activeProduct} thumb={activeThumb} storeSlug={storeSlug} onClose={() => setActiveIdx(null)} />}
    </section>
  );
}

function VideoPlayerModal({
  video, product, thumb, storeSlug, onClose,
}: {
  ...
  return (
    ...
    >
      ...
      >
        {video.video_url ? (
          <video src={video.video_url} autoPlay controls playsInline className="h-full w-full object-contain" />
        ) : null}
...
/* -------------- Scoped CSS -------------- */
export function TheShoesStyles() {
  return (
    <style>{`
